...
import time
#
done_fet = machine.Pin(20, machine.Pin.OUT) # drives the gate of an N-FET which allows the done charging LED to operate
#                                            
vbus_good = machine.Pin(28, machine.Pin.IN) # Pico reads a voltage divider to let it know if the 5 volt power is good
#
pico_led = machine.Pin(25, machine.Pin.OUT) # The onboard LED on the Pico which is used to indicate if the alarm is set
#
white_led = machine.PWM(machine.Pin(0)) # controls the base of an NPN transistor which will turn on/off a white LED
white_led.freq(1000)                    # PWM frequency for white LED
#
BuzzerObj=machine.PWM(machine.Pin(19)) # Controls the base of an NPN transistor which will turn on/off a buzzer
#
light_sensor = machine.ADC(0) #analog value from voltage divider with a light sensitive resistor 
#
pir_sensor = machine.Pin(18, machine.Pin.IN) # passive infrared sensor sends a high to this input when it detects movement
#
...
while True: # endless loop
    if set_time.value() == True: # set and display time
        ...
        if ((pir_sensor.value() == True) & (light_reading >= 33000) & (vbus_good.value() == True)): # Motion and dark room and good pwr
...
# check for motion or room light turned on or bad power. Any one causes 4 digits of clock to be displayed 
        if ((pir_sensor.value() == True) | (light_reading <= 33000) | (vbus_good.value() == False)):
...
#
    if alarm_on.value() == False: # alarm is off
        ...
        if (minute_alarm == minute_counter) & (hour_alarm == hour_counter) & (buzzed == False):
            ...
            BuzzerObj.deinit() #Deactivates the buzzer
            buzzed = True
    if (light_reading >= 33000): # dark room
        done_fet.value(0) # turn off fet so led for battery charge complete is always off 
    else:
        done_fet.value(1) # turn on fet so the led can indicate charge complete   